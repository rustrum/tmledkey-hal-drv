...
fn main() -> ! {
    ...
    if cfg!(feature = "dioclkstb") {
        let mut tm_stb = gpiob.pb7.into_push_pull_output(&mut gpiob.crl);
        let mut tm_clk = gpiob.pb8.into_push_pull_output(&mut gpiob.crh);
        let mut tm_dio = gpiob.pb9.into_open_drain_output(&mut gpiob.crh);
        demo_3wire(&mut delay, &mut led, &mut tm_dio, &mut tm_clk, &mut tm_stb)
    } else {
        let mut tm_clk = gpiob.pb5.into_push_pull_output(&mut gpiob.crl);
        let mut tm_dio = gpiob.pb6.into_open_drain_output(&mut gpiob.crl);
        demo_2wire(&mut delay, &mut led, &mut tm_dio, &mut tm_clk)
    }
}

fn demo_2wire<LED, DIO, CLK>(delay: &mut Delay, led: &mut LED, dio: &mut DIO, clk: &mut CLK) -> !
where
    LED: OutputPin,
    ...
    CLK: OutputPin,
{
    ...
    loop {
        ...
        match read {
            ...
            Err(e) => {
                hprintln!("Key scan read error {:?}", e);
            }
        };
    ...
    STB: OutputPin,
{
    ...
    hprintln!("Starting 2 wire demo (TM1638)");
